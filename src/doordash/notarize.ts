...
import { NotarizationRequestInterface, start } from "../../modules/my-rust-module";

//`/v2/consumers/me/`

export const notarizeDoorDashRequest = (token: string, path: string, redactStrings: string[] = []): NotarizationRequestInterface =>
({
    host: DOORDASH_HOST,
    path,
    body: "",
    headers: [
        ...
        ...DEFAULT_DOORDASH_HEADERS
    ],
    redact_strings: [`JWT ${token}`, ...redactStrings]
})


export const getProfileRedactStrings = (profile: ProfileInterface): string[] => (
    [
        profile.email,
        profile.last_name,
        profile.first_name,
...
export const notarizeDoorDashProfileRequest = (token: string, redactStrings: string[] = []) =>
    notarizeDoorDashRequest(token, `/v2/consumers/me/`, redactStrings)


export const generateDoorDashProof = async (request: NotarizationRequestInterface): Promise<String> => {
